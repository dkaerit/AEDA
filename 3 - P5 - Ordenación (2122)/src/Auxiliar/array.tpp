...
class Array {
    ...
    public:
        ...
        Array(size_t, const type& value = type()); // constructor (0,max)
        Array(size_t, size_t, const type& value = type()); // constructor (min,max)
        Array(const Array<type>&) { base = dmap<type>(); } // constructor copia
        ...
        void swap(unsigned, unsigned); // intercambia dos elementos
...
template<class type>
Array<type>::Array(size_t max, const type& value):
base(max), zero(0)  {
...
template<class type>
Array<type>::Array(size_t min, size_t max, const type& value):
...
/*
 * #brief Intercambia dos elementos del array
 * #tparam type Tipo de valores que almacena el array
...
template<class type>
void Array<type>::swap(unsigned i, unsigned j) {
    if(sublim() > i || i >= suplim() || sublim() > j || j >= suplim()) 
        if(sublim()  > i) throw system_error(errno, system_category(), "swap(i,j) fuera de los limites");

    type c = this->operator[](i);
