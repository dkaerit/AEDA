...
int protectedMain(int argc, char* argv[]) {
    ...
    auto fd = chooseDispersionFn<int>(ts);
    auto fe = chooseDispersionType()?  nullptr : chooseExplorationFn<int>(ts);
...
void menu(unsigned ts, DispersionFn<Key>* fd, ExplorationFn<Key>* fe, unsigned bs) {
    int cas, key;
    HashTable<Key,Block<Key>> htb(ts, fd, fe, bs);
    HashTable<Key,List<Key>> htl(ts, fd);
    ...
    do {
        ...
        switch(cas) {
            case 1: {
                cout << "Ingrese la clave: "; cin >> key; getchar();
                fe? htb.insert(key): htl.insert(key);
            }; break;
            case 2: {
                cout << "Ingrese la clave: "; cin >> key; getchar();
                fe? htb.search(key): htl.search(key);
            }; break;
        }
    } while(cas != 3);
}