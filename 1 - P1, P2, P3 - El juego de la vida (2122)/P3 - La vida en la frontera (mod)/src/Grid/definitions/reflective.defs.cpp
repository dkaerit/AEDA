...
GridWithReflectiveBorder::GridWithReflectiveBorder(int x, int y, State* st, int t): 
Grid(x+2, y+2, t) {
    ...
    for(auto i = 0; i < _n; i++) {
        for(auto j = 0; j < _m; j++) {
            ...
            if(isMargin(i,j)) {
                if(i == 0 && j == 0) _grid[i][j].setPos(  1,   1);
                if(i == 0 && j == m) _grid[i][j].setPos(  1, m-1); 
                if(i == n && j == 0) _grid[i][j].setPos(n-1, m-1); 
                if(i == n && j == m) _grid[i][j].setPos(n-1, m-1);

                if(i == 0 && (0 < j && j < m)) _grid[i][j].setPos(  1,   j); 
                if(j == 0 && (0 < i && i < n)) _grid[i][j].setPos(  i,   1);
                if(i == n && (0 < j && j < m)) _grid[i][j].setPos(n-1,   j);
                if(j == m && (0 < i && i < n)) _grid[i][j].setPos(  i, m-1);
 ...
 * @param os 
 */
void GridWithReflectiveBorder::frame(int i, int j, std::ostream& os) const {
    int x = _m-1, y=_n-1;
