...
_base((max-min+1)), _zero(min) {
    _base = std::deque<T>((max-min+1), fill);
}
...
/**
 * 
 * @verbatim Sobrecarga de operadores
 * @endverbatim
 * 
 * */
...
T& evector<T>::operator[](const int i)       { 
    if(sublim() <= i && i < suplim()) 
        return _base[i-_zero]; 
    else {
        if(sublim()  > i) throw _base[i-_zero+1];
        if(suplim() <= i) throw _base[i-_zero-1];
    } 
}   

template<class T>
T& evector<T>::operator[](const int i) const {
    if(sublim() <= i && i < suplim()) 
        return _base[i-_zero]; 
    else {
        if(sublim()  > i) throw _base[i-_zero+1];
